...
class PromptEov:
    def __init__(self, initial_prompt:str = INITIAL_PROMPT, dataset:list[dict] = None, *, analyzer_prompt=DEFAULT_ANALYZER, optimizer_prompt=DEFAULT_OPTIMIZER, generate_query:Callable[[str,str],str]|None=None, retrieve:Callable[[str],dict]=test_retrieval, llm:Callable[[str],str]|None=None, temperature:float=0.0):
        if isinstance(temperature, bool) or not isinstance(temperature, (int, float)) or not 0 <= temperature <= 2:
            raise ValueError('temperature must be between 0 and 2')
        self.initial_prompt,self.dataset=initial_prompt,dataset or []; self.analyzer_prompt,self.optimizer_prompt=analyzer_prompt,optimizer_prompt; self.generate_query=generate_query; self.retrieve=retrieve; self.llm=llm; self.temperature=temperature
    ...
    def _gen(self, prompt, dialogue):
        ...
        out=self._call_llm(prompt.format(dialogue=dialogue)); m=re.search(r'"query"\s*:\s*"(.*?)"',out,re.S); return m.group(1) if m else out.strip()
    def run(self, iterations=3, output_dir='promptEov/runs'):
        ...
        for i in range(1,iterations+1):
            ...
            analysis=self._call_llm(self.analyzer_prompt.format(bad_cases=json.dumps(bad,ensure_ascii=False,indent=2))) if self.llm else ''
            new_prompt=self._call_llm(self.optimizer_prompt.format(initial_prompt=self.initial_prompt,analysis=analysis)) if self.llm else prompt
